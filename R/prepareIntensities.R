...
prepareComparisonExperiment <- function(intensityDataset,
                                        conditionComparison,
                                        conditionCol,
                                        replicateCol,
                                        featureIdCol,
  ...
  intensitiesDf <- intensityDataset@intensitiesTable
  # Replicates and conditions
  replicateConditionMapping <- unique(intensitiesDf[,c(conditionCol,replicateCol)])
  uniqueConditions <- unique(replicateConditionMapping[,conditionCol, drop=TRUE])
  if(!(leftCondition %in% uniqueConditions) | !(rightCondition %in% uniqueConditions)){
    stop(glue("The up and down conditions ({leftCondition}, {rightCondition}) provided are not present in the samples data."))
  }

  info(EnrichmentLogger(), "Filter intensity table by missing value.")
  filteredData <- filterByMissingProportion(intensityDataset = intensityDataset,
  ...
  proteinIntensitiesWide <- makeWideIntensitiesDf(intensityDataset = filteredData,
                                                  groupByCol = "ProteinIds",
                                                  intensityCol = intensityCol,
                                                  replicateCol = replicateCol)

  info(EnrichmentLogger(), "Create comparison experiment.")
...
makeWideIntensitiesDf <- function(intensityDataset, groupByCol = "ProteinIds",
                                  intensityCol = "NormalisedIntensity", replicateCol = "replicate"){

  proteinIntensities <- data.frame(intensityDataset@intensitiesTable)
  metadataTable <- data.frame(intensityDataset@metadataTable)
  proteinIntensities <- proteinIntensities %>% left_join(metadataTable[,c("GroupId", groupByCol)])
